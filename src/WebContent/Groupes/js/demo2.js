...
function joinFirepadForHash() {
  ...
  var id = window.location.hash.replace(/#/g, '') || randomString(10); //il prend se qui vien apres le # s'il n y a pas il génere un mot aleatoire
  var url = window.location.toString().replace(/#.*/, '') + '#' + id;
  var firepadRef = new Firebase('https://intense-fire-2283.firebaseio.com/compte').child(id);

  var userId = firepadRef.push().name(); // Just a random ID.
  codeMirror = CodeMirror(document.getElementById('firepad2'), { lineWrapping: true });
